...
// console.log(searchText);
   let  searchResultData = [] as IDataList[];
   
    
const [ favouriteList, setFavouritelist ] = useState<{ movie: IDataList, favourite : number }[]>( [] );
...
const addToFavourite = (movieValue : IDataList) =>
{
    const movie = {
    ...
    } as Omit<IDataList, 'id | poster | duration| releaseDate |  originalTitle'>;
...
const removeFromFavourite = (movie : IDataList) =>
{
    const  newFavourite = [] as { movie: IDataList, favourite : number  }[];
    ...
    data[typeOfMovie].forEach(
        p => {
            ...
            if(  p.title.toLowerCase().startsWith(searchText.toLowerCase())) {
                searchResultData.push( p as IDataList);
            }
        }
    );
    }
 ...
 else
 {
    ...
    {
        searchResultData = data[typeOfMovie] as IDataList[];
    }
 }

 const showMovieDetails = (movie : IDataList) =>
 {
